/*
 * Unit test for MSCKF.
 * Use KITTI dataset to propagate(gyroscope only), simulation landmarks to update.
...
void LogTrajectory(const MSCKF2& ekf, vector<vec3>* loc, vector<vec4>* ori) {
...
int main(int argc, char* argv[]) {
   ...
   ****************************************************************************/
  MSCKF2 ekf_ppg_only(vtracker->virtual_camera().R_cam_in_imu.transpose(),
                      vtracker->virtual_camera().p_cam_in_imu);
  MSCKF2 ekf(vtracker->virtual_camera().R_cam_in_imu.transpose(),
             vtracker->virtual_camera().p_cam_in_imu);
  ...
  while (has_imu && imu_counter < stop - start) {
    ...
    ekf.propagate(t_second, gyro);
    ekf_ppg_only.propagate(t_second, gyro);
